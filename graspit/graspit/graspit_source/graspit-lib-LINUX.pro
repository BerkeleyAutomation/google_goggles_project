...
cgdb {
        graspit_ros {        
                ...
                DATABASE_INTERFACE_CFLAGS = $$system(rospack export --lang=cpp --attrib=cflags database_interface)
                QMAKE_CXXFLAGS += $$DATABASE_INTERFACE_CFLAGS $$MODEL_DATABASE_CFLAGS 

                MODEL_DATABASE_LFLAGS = $$system(rospack export --lang=cpp --attrib=lflags household_objects_database)
                DATABASE_INTERFACE_LFLAGS = $$system(rospack export --lang=cpp --attrib=lflags database_interface)
                QMAKE_LFLAGS += $$DATABASE_INTERFACE_LFLAGS $$MODEL_DATABASE_LFLAGS
        }
}
