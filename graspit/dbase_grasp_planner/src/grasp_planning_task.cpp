...
void GraspPlanningTask::start()
{
  ...
  Body* table = 0;
  int num_bodies = world->getNumBodies();
  ...
  if (table) {
	world->toggleCollisions(false,table,mObject);
  }
  ...
  //DBGA("MAX STEPS SET LOW, CHANGE BACK");
...
void GraspPlanningTask::plannerLoopUpdate()
{
  ...
  DBGA("Found " << mPlanner->getListSize() << " solutions, checking pregrasps");
  ...
  if (mStatus == ERROR) {
    ...
    DBGA(num_saved << " solutions saved to database");

    std::vector<db_planner::Grasp*> grasp_list;
    
    mDBMgr->GetGrasps(*mObject->getDBModel(),"WILLOW_GRIPPER_2010",&grasp_list);
	
	//std::cout << "grasp list size: " << grasp_list.size() << std::endl;
